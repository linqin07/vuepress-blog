...
![image-20220303231205713](https://gitee.com/linqin07/pic/raw/master/image-20220303231205713.png)

#### 2.Block Nested-Loop Join

一条条记录取出重复太多，增加一个JoinBuffer缓存池，遍历被驱动表时一次性比较，增加速度。5.8版本使用 hash join 代替，建立匹配结果集小的驱动表的散列表，扫描大表进行hash匹配。

![image-20220303232720490](https://gitee.com/linqin07/pic/raw/master/image-20220303232720490.png)

#### 3.Index Nested-Loop Join

对关联字段进行索引后，驱动表查找速度就直线上升，不用遍历被驱动表了。只需要从索引中获取到对应的值后通过回表操作取出匹配的数据集合。

![image-20220303233305170](https://gitee.com/linqin07/pic/raw/master/image-20220303233305170.png)
...
2. 结果集小的表 驱动 结果集大的表，本质就是减少外层循环的次数
3. 为匹配的条件增加索引
...
![image-20220307235947950](https://gitee.com/linqin07/pic/raw/master/image-20220307235947950.png)
...
![image-20220308001024295](https://gitee.com/linqin07/pic/raw/master/image-20220308001024295.png)
